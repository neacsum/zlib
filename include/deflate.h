/*!
  \file deflate.h Internal compression state
...
typedef struct internal_state {
    ...
    int   last_flush;    /*!< value of flush param for previous deflate call */
                ...
                /*! \name used by deflate.c: */
    ///@{
    uInt  w_size;        /*!< LZ77 window size (32K by default) */
    uInt  w_bits;        /*!< log2(w_size)  (8..16) */
    uInt  w_mask;        /*!< w_size - 1 */

    Bytef *window;       /*!< Sliding window. Input bytes are read into the
                         second half of the window, and move to the first half
                         later to keep a dictionary of at least wSize bytes.
                         
                         With this organization, matches are limited to a distance of
    ...
    Posf *prev;          /*!< Link to older string with same hash index. To limit
    ...
    uInt  hash_shift;    /*!< Number of bits by which ins_h must be shifted at
                          each input step. It must be such that after MIN_MATCH
                          steps, the oldest byte no longer takes part in the hash
                          key, that is: hash_shift * MIN_MATCH >= hash_bits */

    long block_start;    /*!< Window position at the beginning of the current
                         output block. Gets negative when the window is moved
                         backwards. */
